...
public class ProductDaoImpl extends connectDB implements ProductDao {
	...
	public List<Product> getProductSelling() {
		List<Product> products = new ArrayList<Product>();
		String sql ="select product.id, product.catalog_id, product.name, product.price, product.status, product.description, product.content, product.discount, product.image_link, product.created from product inner join ordered on product.id = ordered.product_id order by ordered.qty desc limit 4";
		Connection conn = connectDB.getConnect();
