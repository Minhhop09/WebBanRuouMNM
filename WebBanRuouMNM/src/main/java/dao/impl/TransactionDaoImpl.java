...
public class TransactionDaoImpl extends connectDB implements TransactionDao {
	...
	public List<Transactions> getAllbyUser(String user_session) {
		List<Transactions> transactions = new ArrayList<Transactions>();
		String sql = "SELECT * FROM transactions WHERE user_session like ? ";
		Connection conn = connectDB.getConnect();
		...
		try {
			...
			while (rs.next()) {
				...
				transaction.setUser_session(rs.getString(2));
				transaction.setUser_name(rs.getString(3));
				transaction.setUser_mail(rs.getString(4));
				transaction.setUser_phone(rs.getString(5));
				transaction.setAddress(rs.getString(6));
				transaction.setMessage(rs.getString(7));
				transaction.setAmount(rs.getString(8));
				transaction.setPayment(rs.getString(9));
				transaction.setStatus(rs.getString(10));
				transaction.setCreated(rs.getString(11));
				transactions.add(transaction);
			}
	...
	public List<Transactions> getByDay() {
		List<Transactions> transactions = new ArrayList<Transactions>();
		String sql = "select * from transactions where created=convert(date, getdate())";
		Connection conn = connectDB.getConnect();
	...
	public List<Transactions> getByMonth() {
		List<Transactions> transactions = new ArrayList<Transactions>();
		String sql = "select *from transactions where YEAR(created) =YEAR(convert(date, getdate())) and MONTH(created) =MONTH(convert(date, getdate()))";
		Connection conn = connectDB.getConnect();
